...
const supabase = (() => {
  ...
  const api = (path, opts = {}) => fetch(`${SUPABASE_URL}/rest/v1/${path}`, { headers: { ...headers, ...(opts.headers || {}) }, ...opts });
  const authApi = (path, body) => fetch(`${SUPABASE_URL}/auth/v1/${path}`, { method: "POST", headers: { "Content-Type": "application/json", apikey: SUPABASE_ANON_KEY }, body: JSON.stringify(body) });
  return {
...
function SignUpScreen({ onNav, onAuth }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (data.error) return setMsg({ text: data.error.message || "Sign up failed.", type: "error" });
    ...
    } catch { setMsg({ text: "Connection error. Check your Supabase credentials.", type: "error" }); }
    setLoading(false);
  };
...
function SignInScreen({ onNav, onAuth }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.error) return setMsg({ text: "Invalid credentials. Please try again.", type: "error" });
      const token = data.access_token;
      const uid = data.user?.id;
...
function ForgotScreen({ onNav }) {
  ...
  const submit = async () => {
    ...
    } catch { setMsg({ text: "Error sending reset email.", type: "error" }); }
    setLoading(false);
  };
...
function HealthProfileScreen({ user, onNav, onSaved }) {
  ...
  const save = async () => {
    ...
    } catch { setMsg({ text: "Error saving. Check your Supabase connection.", type: "error" }); }
    setLoading(false);
  };
...
function SetupGuide({ onDismiss }) {
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ background: C.white, borderRadius: 20, padding: 24, maxWidth: 400, width: "100%", maxHeight: "80vh", overflowY: "auto" }}>
        <div style={{ fontSize: 18, fontWeight: 800, color: C.text, marginBottom: 4 }}>🚀 Setup Guide</div>
        <div style={{ fontSize: 13, color: C.gray, marginBottom: 16 }}>Connect your Supabase database in 3 steps:</div>
        {[
          { n: 1, t: "Create Supabase Project", d: "Go to supabase.com → New Project → Copy your Project URL and anon key from Settings → API" },
          { n: 2, t: "Create Database Tables", d: 'In Supabase SQL Editor, run the SQL below to create your tables.' },
          { n: 3, t: "Paste Credentials", d: 'Replace YOUR_SUPABASE_URL and YOUR_SUPABASE_ANON_KEY at the top of this app\'s code.' },
        ].map(s => (
          <div key={s.n} style={{ display: "flex", gap: 12, marginBottom: 16 }}>
            <div style={{ width: 28, height: 28, borderRadius: "50%", background: C.primary, color: C.white, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: 14, flexShrink: 0 }}>{s.n}</div>
            <div><div style={{ fontWeight: 700, fontSize: 14, color: C.text, marginBottom: 2 }}>{s.t}</div><div style={{ fontSize: 12, color: C.gray }}>{s.d}</div></div>
          </div>
        ))}
        <div style={{ background: "#1e293b", borderRadius: 10, padding: 12, fontSize: 11, color: "#7dd3fc", fontFamily: "monospace", marginBottom: 16, overflowX: "auto" }}>
...
create table "HealthProfile" (
  id uuid default gen_random_uuid() primary key,
  user_id uuid references auth.users on delete cascade,
  conditions text, symptoms text,
  height text, weight text,
  blood_pressure text, blood_type text,
  allergies text, medications text,
  smoking text, alcohol text,
  exercise text, notes text,
...
create policy "Users manage own profile" on "UserProfile"
  for all using (auth.uid() = user_id);
create policy "Users manage own health" on "HealthProfile"
  for all using (auth.uid() = user_id);
create policy "All read health" on "HealthProfile"
  for select using (true);`}
        </div>
        <Btn label="Got it — Let's Go!" onClick={onDismiss} />
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  return (
    <div style={{ fontFamily: "'Segoe UI', system-ui, sans-serif", maxWidth: 480, margin: "0 auto", background: C.lightGray, minHeight: "100vh" }}>
      {showSetup && SUPABASE_URL === "YOUR_SUPABASE_URL" && <SetupGuide onDismiss={() => setShowSetup(false)} />}
